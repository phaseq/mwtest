...
class Dialog(Tkinter.Frame):
    ...
    def poll(self):
        ...
        if len(now) > 0 and self.prev_now != now:
            ...
            self.label.insert(Tkinter.END, test["system_out"])
    ...
    for testsuite in testsuites:
        ...
        for test in testsuite.getElementsByTagName('testcase'):
            ...
            if not os.path.isabs(reference_path):
                tests_path = doc.getElementsByTagName('reference_root')[0].firstChild.nodeValue
                reference_path = os.path.abspath(os.path.join(tests_path, reference_path))
            ...
            test_result = {
                ...
                "exit_code": test.getElementsByTagName("exit_code")[0].firstChild.nodeValue,
                "system_out": test.getElementsByTagName("system_out")[0].firstChild.nodeValue,
                "artifacts": artifacts
            }
